...
namespace HoloTable.Data
{
    ...
    public sealed class CardCatalog : ScriptableObject
    {
        ...
        /// <summary>Every definition reachable from this catalog (includes flattened, no duplicates).</summary>
        public IReadOnlyList<EntityDefinition> AllDefinitions()
        {
            var result = new List<EntityDefinition>();
            CollectDefinitions(this, new HashSet<CardCatalog>(), new HashSet<EntityDefinition>(), result);
            return result;
        }

        private static void CollectDefinitions(CardCatalog catalog, HashSet<CardCatalog> visited, HashSet<EntityDefinition> seen, List<EntityDefinition> result)
        {
            ...
            foreach (CardCatalog child in catalog.includes) CollectDefinitions(child, visited, seen, result);
        }
