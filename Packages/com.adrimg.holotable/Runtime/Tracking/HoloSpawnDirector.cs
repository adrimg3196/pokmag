...
namespace HoloTable.Tracking
{
    ...
    public sealed class HoloSpawnDirector : MonoBehaviour
    {
        ...
        [Tooltip("Build a default HUD in code when no HUD prefab is assigned.")]
        [SerializeField] private bool createDefaultHud = true;
        ...
        public LivingEntityController SpawnEntity(EntityDefinition definition, TrackedTarget target, PlayerSide side, bool playSpawn = true)
        {
            if (definition == null)
            {
                Debug.LogWarning("[HoloTable] SpawnEntity called with a null definition.", this);
                return null;
            }

            Vector3 position = target != null && target.HasAnchor ? target.Anchor.position : transform.position;
            LivingEntityController entity;
            if (definition.Prefab != null)
            {
                entity = Instantiate(definition.Prefab, position, Quaternion.identity);
            }
            else
            {
                entity = PlaceholderHologramFactory.Create(definition, position);
                Log($"'{definition.DisplayName}' has no prefab: using a procedural placeholder hologram.");
            }
            ...
            else if (createDefaultHud) entity.AttachHud(EntityHUD.CreateDefault());
        ...
        /// <summary>Hologram currently bound to a tracked card, or null.</summary>
        public LivingEntityController GetEntity(string instanceId) =>
            !string.IsNullOrEmpty(instanceId) && _entries.TryGetValue(instanceId, out TargetEntry e) ? e.Entity : null;
